...
class obama(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        if obama.image is None:
            obama.image=pygame.image.load('obama2.jpg')
...
# pervez
class pervez(pygame.sprite.Sprite):
        image = None

        def __init__(self):
            pygame.sprite.Sprite.__init__(self)
            if pervez.image is None:
                pervez.image = pygame.image.load('pervez2.jpg')
            self.image = pervez.image
            # update the coordinates of the image
            self.rect = self.image.get_rect()
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    if xin == 1:
                        ...
                        per.locate(monst_startx, monst_starty)
                        obm.locate(xrout, yrout)
                        putin.locate(xrout, yrout)
                        bush.locate(xrout, yrout)
